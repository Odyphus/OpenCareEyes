...
- `pyproject.toml` 是版本号唯一来源，版本为 `0.7.0`。
...
- README 的白鼬预览来自实际随包资源；v0.4 历史截图/GIF 明确标为旧界面，不能冒充 v0.7“伙伴小屋”。
...
git switch main
git pull --ff-only
$Version = "v0.7.0" # 必须与 pyproject.toml 一致
...
- `OpenCareEyes.exe`
- `OpenCareEyes_Portable_0.7.0.zip`
- `OpenCareEyes_Setup_0.7.0.exe`
- `SHA256SUMS.txt`
- `OpenCareEyes_WinGet_0.7.0.zip` 候选清单
...
1. Tag、Release 标题和 `pyproject.toml` 都是 `0.7.0`。
2. 六个发布资产可下载，文件名无误。
3. `SHA256SUMS.txt` 与实际便携版、安装版、WinGet ZIP 和第三方 notice 完全匹配。
4. 在干净 Windows 环境完成便携启动、单实例唤起、安装、从正式发布的 v0.4.1 升级和卸载；确认应用名、AppId 与用户设置路径保持兼容。
...
python scripts\generate_winget_manifest.py `
  .\installer_output\OpenCareEyes_Setup_0.7.0.exe `
  --version 0.7.0

winget validate .\winget_output\manifests\o\Odyphus\OpenCareEyes\0.7.0
```

然后按 [Microsoft WinGet 提交流程](https://learn.microsoft.com/windows/package-manager/package/repository) 完成：

1. 检查 `InstallerUrl` 指向版本固定的 `v0.7.0` GitHub Release 资产，不使用 `/latest/`。
