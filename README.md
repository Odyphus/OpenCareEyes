...
[![Version](https://img.shields.io/badge/version-0.3.0-5B8DEF.svg)](CHANGELOG.md)
...
- 智能免打扰：全屏、演示、独占 D3D、空闲、锁屏或睡眠时暂时让出休息/专注效果；情境结束后自动读取最新设置恢复。
- 应用例外：只保存小写 EXE 文件名，可分别控制休息、专注、色温和调暗；不保存窗口标题、完整路径或使用历史。
- 轻量桌宠：状态切换采用短动画，工作状态低频眨眼；隐藏或被情境抑制后停止动画，并支持系统“减少动画”。
- Windows 体验：单实例启动会唤起已有窗口，支持系统托盘、开机自启、亮色/暗色/跟随系统主题和多显示器。
- 本地优先：无账号、无云同步、无广告、无遥测；设置和诊断信息保存在本机。

完整变更见 [CHANGELOG.md](CHANGELOG.md)。每显示器独立方案、平滑色温和完整规则编辑器留待后续版本；v0.3 不读取麦克风/音频，也不宣称能判断 Teams 或 Zoom 是否正在通话。

## 界面预览

![OpenCareEyes 亮色总览](docs/images/overview-light.png)
![OpenCareEyes 暗色总览](docs/images/overview-dark.png)
![OpenCareEyes v0.3 30 秒演示](docs/images/OpenCareEyes-v0.3-demo.gif)
...
- 在“自动化”中可开关智能免打扰、全屏暂停和自然休息，也可把最近使用的外部应用加入逐功能例外。
- 总览、自动化页和托盘会显示当前暂停原因及恢复条件；非锁屏/睡眠场景可选择“本次场景继续提醒”。
...
| 配置 | Qt `QSettings`，schema v3（从 v1/v2 无损迁移） |

Windows 10/11 是 v0.3 的唯一受支持平台。Gamma Ramp 能力取决于显卡驱动、远程桌面和显示设备；不支持时应用应给出降级提示。
